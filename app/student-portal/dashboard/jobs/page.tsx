...
interface AlumniJob {
  ...
  Deadline_Unix: string | number | null;
  External_Link: string | null;
  Description_HTML: string | null;
}

function formatDeadline(unixValue: string | number | null): string {
  if (unixValue == null || unixValue === '') return 'Not specified';
  const parsed = Number(unixValue);
  if (!Number.isFinite(parsed) || parsed <= 0) return 'Not specified';
  const date = new Date(parsed * 1000);
  if (Number.isNaN(date.getTime())) return 'Not specified';
  return date.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' });
}

function formatExperience(min: string | null, max: string | null): string {
...
function formatSalary(text: string | null, min: string | null, max: string | null): string {
  if (text) return text;
...
export default function JobsPage() {
  ...
  const [alumniJobs, setAlumniJobs] = useState<AlumniJob[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (active) {
          setJobs(data.jobs ?? []);
          setAlumniJobs(data.alumni_jobs ?? []);
        }
      } catch { /* silent */ }
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      {loading ? (
      ...
      ) : jobs.length === 0 && alumniJobs.length === 0 ? (
        ...
        <div className="space-y-6">
          {jobs.length > 0 && (
            <div>
              <h2 className="text-sm font-bold text-[#1f2a44] mb-2">Campus Placement Jobs</h2>
          ...
          {alumniJobs.length > 0 && (
            <div>
              <h2 className="text-sm font-bold text-[#1f2a44] mb-2">Alumni Network Jobs</h2>
              <div className="grid gap-3">
                {alumniJobs.map((job) => (
                  <div key={job.External_Id} className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <h3 className="text-sm font-bold text-gray-900">{job.Designation || 'Job Opportunity'}</h3>
                        <p className="text-xs text-gray-500">{job.Company_Name || 'Company not specified'}</p>
                      </div>
                      <span className="px-2 py-0.5 rounded-full text-[11px] font-semibold bg-[#2E3093]/10 text-[#2E3093]">External</span>
                    </div>

                    <div className="flex flex-wrap gap-3 mt-2">
                      <span className="text-[11px] text-gray-500">Experience: {formatExperience(job.Experience_Min, job.Experience_Max)}</span>
                      <span className="text-[11px] text-gray-500">Salary: {formatSalary(job.Salary_Text, job.Salary_Min, job.Salary_Max)}</span>
                      <span className="text-[11px] text-gray-500">Deadline: {formatDeadline(job.Deadline_Unix)}</span>
                    </div>
                    ...
                    {job.External_Link ? (
                      <div className="mt-3">
                        <a
                          ...
                          className="inline-flex items-center px-3 py-1 text-[11px] font-semibold text-white bg-[#2E3093] hover:bg-[#252780] rounded transition-colors"
                        >
                          Open Job Link
                        </a>
                      </div>
