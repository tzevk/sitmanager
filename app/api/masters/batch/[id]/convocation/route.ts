...
export async function GET(
  ...
  try {
    ...
    const [rows] = await pool.query<RowDataPacket[]>(
      `SELECT id, faculty_name, guest_name, guest_mobile, email, guest_designation
    ...
    return NextResponse.json({ error: 'Failed to fetch convocations' }, { status: 500 });
  }
}
...
export async function POST(
  ...
  try {
    ...
    const [result] = await pool.query<ResultSetHeader>(
      `INSERT INTO batch_convocation (batch_id, faculty_name, guest_name, guest_mobile, email, guest_designation, deleted)
       VALUES (?, ?, ?, ?, ?, ?, 0)`,
    ...
    return NextResponse.json({ error: 'Failed to save convocation' }, { status: 500 });
  }
}
...
export async function PUT(
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to update convocation date' }, { status: 500 });
  }
}

export async function DELETE(request: NextRequest) {
  try {
    const convocationId = request.nextUrl.searchParams.get('convocationId');

    if (!convocationId) return NextResponse.json({ error: 'Convocation ID required' }, { status: 400 });

    const pool = getPool();
    await pool.query(`UPDATE batch_convocation SET deleted = 1 WHERE id = ?`, [convocationId]);

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('Error deleting convocation:', error);
    return NextResponse.json({ error: 'Failed to delete convocation' }, { status: 500 });
  }
}
