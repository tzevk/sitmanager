...
async function runSync(req: NextRequest) {
  ...
  // sinceHours=0 (or unset) → full backfill of all legacy records
  ...
  const sinceHours =
    Number.isFinite(sinceHoursRaw) && sinceHoursRaw > 0 ? sinceHoursRaw : undefined;

  const batchSizeRaw = Number(
    req.nextUrl.searchParams.get('batchSize') ||
    process.env.LEGACY_INQUIRY_SYNC_BATCH_SIZE ||
    '200'
  );
  const batchSize =
    Number.isFinite(batchSizeRaw) && batchSizeRaw > 0 ? Math.trunc(batchSizeRaw) : 200;

  const summary = await syncLegacyInquiries({ sinceHours, batchSize });

  if (!summary.configured) {
    return NextResponse.json({
      ok: false,
      message: 'OLD_DB_HOST is not configured — legacy inquiry sync skipped',
    });
  }
