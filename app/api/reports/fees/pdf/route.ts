...
export async function GET(req: NextRequest) {
  try {
    ...
    students.forEach((stu, idx) => {
      ...
      drawCell(colX[0], y, colWidths[0], rowH, String(idx + 1), { align: 'center' });
      drawCell(colX[1], y, colWidths[1], rowH, stu.Student_Name || '—', { bold: true });
      drawCell(colX[2], y, colWidths[2], rowH, fmtMoney(amount), { align: 'right' });
      drawCell(colX[3], y, colWidths[3], rowH, fmtMoney(paid), { align: 'right' });
      drawCell(colX[4], y, colWidths[4], rowH, fmtMoney(rem), { align: 'right' });
      y += rowH;
    });
