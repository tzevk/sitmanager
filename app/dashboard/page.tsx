                ...
                {(data.notices ?? []).map((n: any) => (
                  <div key={n.id} className="p-3 rounded-xl bg-purple-50/50 border border-purple-100/50">
                    ...
                    <div className="flex items-center gap-2 mt-2 text-[10px] text-gray-400">
                      {n.startdate && <span>From: {n.startdate}</span>}
                      {n.enddate && <span>To: {n.enddate}</span>}
                      {n.created_date && <span className="ml-auto">{new Date(n.created_date).toLocaleDateString()}</span>}
                    </div>
                  </div>
                ))}
              </div>
              ...
              {(data.enquiryReport?.recentCorporate ?? []).length > 0 ? (
                (data.enquiryReport?.recentCorporate ?? []).map((e: any) => (
                  <div key={e.Id} className="flex items-center gap-3 p-2.5 rounded-xl bg-gray-50 hover:bg-gray-100/80 transition-colors">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-[13px] font-medium text-gray-700 truncate">{e.FullName}</p>
                      <p className="text-[11px] text-gray-400 truncate">{e.CompanyName}{e.Designation ? ` · ${e.Designation}` : ''}</p>
                    </div>
                    <span className="text-[10px] text-gray-300 whitespace-nowrap">{e.Idate || ''}</span>
                  </div>
                ))
              ) : (
                <p className="text-gray-300 text-sm text-center py-4">No recent enquiries</p>
              )}
            </div>
          </div>
        </Widget>
                ...
                {(data.placementReport?.companyRequirements ?? []).map((r: any) => (
                  <div key={r.CompReqId} className="p-3 rounded-xl border border-gray-100 hover:border-sky-200 hover:shadow-sm transition-all">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <p className="text-[13px] font-semibold text-gray-800 truncate">{r.Profile || 'Open Position'}</p>
                        <p className="text-[11px] text-gray-400 mt-0.5">{r.CompanyName || 'Company'}{r.Location ? ` · ${r.Location}` : ''}</p>
                      </div>
                      {r.PostedDate && <span className="text-[10px] text-gray-300 whitespace-nowrap">{r.PostedDate}</span>}
                    </div>
                    {r.Eligibility && <p className="text-[11px] text-gray-500 mt-1.5 line-clamp-2">{r.Eligibility}</p>}
                  </div>
                ))}
              </div>
            ) : (
