...
export default function CVShortlistedPage() {
  const router = useRouter();
  const { canView, canCreate, canUpdate, canDelete, loading: permLoading } = useResourcePermissions('cv_shortlisted');

  const [rows, setRows] = useState<CVShortlisted[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admission-activity/cv-shortlisted?${params.toString()}`);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/admission-activity/cv-shortlisted?id=${id}`, { method: 'DELETE' });
      if (res.ok) fetchData();
    } catch { /* ignore */ }
  };
  ...
  const handleExport = () => {
    ...
    a.href = url; a.download = 'cv_shortlisted.csv'; a.click();
  ...
  if (!canView) return <AccessDenied message="You do not have permission to view CV Shortlisted." />;
    ...
    <div className="h-screen overflow-hidden bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/20 p-6 flex flex-col">
      ...
      <div className="mb-6 flex-shrink-0 flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 text-sm text-gray-500 mb-2">
            ...
            <span>Placement</span>
            <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" /></svg>
            <span className="text-[#2E3093] font-medium">CV Shortlisted</span>
          </div>
          <h1 className="text-2xl font-bold text-gray-800">CV Shortlisted</h1>
        </div>
        {canCreate && (
          <button
            onClick={() => router.push('/dashboard/cv-shortlisted/add')}
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col flex-1 min-h-0">
        <div className="bg-gradient-to-r from-[#2E3093] to-[#2A6BB5] px-5 py-3 flex-shrink-0">
          <h3 className="text-sm font-bold text-white tracking-wide">List Of CV Shortlisted</h3>
        </div>
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr key={row.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                      <td className="px-3 py-2.5 text-gray-900 font-medium">{row.Batch_Code || '-'}</td>
                      <td className="px-3 py-2.5 text-gray-700 truncate max-w-[200px]" title={row.Course_Name || ''}>{row.Course_Name || '-'}</td>
                      <td className="px-3 py-2.5 text-gray-700 truncate max-w-[200px]" title={row.CompanyName || ''}>{row.CompanyName || '-'}</td>
                      <td className="px-3 py-2.5 text-gray-700">{formatDate(row.TDate)}</td>
                      <td className="px-3 py-2.5 text-center">
                        <div className="flex items-center justify-center gap-1">
                          {canUpdate && (
                            <button onClick={() => router.push(`/dashboard/cv-shortlisted/edit/${row.id}`)}
