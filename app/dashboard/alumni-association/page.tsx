...
interface AlumniMatch extends AlumniCsvRow {
              ...
              <table className="w-full text-xs border-collapse [&_th]:border-r [&_th]:border-slate-300 [&_th:last-child]:border-r-0 [&_td]:border-r [&_td]:border-slate-200 [&_td:last-child]:border-r-0">
                <thead>
                  <tr className="text-[10px] uppercase tracking-wider text-slate-700 bg-slate-200 border-b border-slate-300">
                    ...
                    <th className="text-left py-2 px-3 font-bold whitespace-nowrap">First Name</th>
                    <th className="text-left py-2 px-3 font-bold whitespace-nowrap">Last Name</th>
                    <th className="text-left py-2 px-3 font-bold whitespace-nowrap">E-mail</th>
                    <th className="text-left py-2 px-3 font-bold whitespace-nowrap">Phone</th>
                    <th className="text-left py-2 px-3 font-bold whitespace-nowrap">secondary_email</th>
                    ...
                    <th className="text-left py-2 px-3 font-bold whitespace-nowrap">Matched Student</th>
                    <th className="text-center py-2 px-3 font-bold whitespace-nowrap">Match Type</th>
                    <th className="text-center py-2 px-3 font-bold whitespace-nowrap">Current Alumni Status</th>
                  </tr>
                </thead>
                <tbody>
                  {preview.matches.length === 0 ? (
                    <tr><td colSpan={12} className="py-8 text-center text-xs text-slate-400">No matches found.</td></tr>
                    ...
                    <tr key={m.studentId} className="border-b border-slate-100 hover:bg-slate-50">
                      ...
                      <td className="py-1.5 px-3 whitespace-nowrap">{m.firstName || '—'}</td>
                      <td className="py-1.5 px-3 whitespace-nowrap">{m.lastName || '—'}</td>
                      <td className="py-1.5 px-3 max-w-[180px]"><span className="truncate block">{m.email || '—'}</span></td>
                      <td className="py-1.5 px-3 font-mono whitespace-nowrap">{m.csvPhone || '—'}</td>
                      <td className="py-1.5 px-3 max-w-[180px]"><span className="truncate block">{m.secondaryEmail || '—'}</span></td>
                      <td className="py-1.5 px-3 whitespace-nowrap">{m.dob || '—'}</td>
                      <td className="py-1.5 px-3 whitespace-nowrap">{m.batchNumber || '—'}</td>
                      <td className="py-1.5 px-3 max-w-[160px]"><span className="truncate block">{m.trainingProgram || '—'}</span></td>
