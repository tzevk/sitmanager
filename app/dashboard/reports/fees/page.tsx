        ...
        <tbody>
          {rows.map((r, i) => (
            <tr key={i} className="hover:bg-slate-50/60 transition-colors">
              ...
              <td className={`${TD} max-w-[180px] truncate`}>{r.Course_Name || '—'}</td>
              <td className={TD}>{fmtDate(r.Batch_Start)}</td>
              <td className={TD}>{fmtDate(r.Batch_End)}</td>
              <td className={`${TD} text-right font-mono`}>{r.Receipt_Count}</td>
              <td className={`${TD} text-right font-mono`}>{r.Student_Count}</td>
              <td className={`${TD} text-right font-mono`}>{fmt(r.Total_Amount)}</td>
              <td className={`${TD} text-right font-mono`}>{r.Total_Tax ? fmt(r.Total_Tax) : '—'}</td>
              <td className={`${TD} text-right font-mono font-semibold text-emerald-700`}>{fmt(r.Total_Collected)}</td>
            </tr>
          ))}
        </tbody>
        <tfoot>
          <tr className="bg-slate-50 border-t-2 border-slate-200">
            <td colSpan={9} className="py-2 px-3 text-xs text-slate-600 text-right font-bold">Total ({rows.length} batches)</td>
            <td className="py-2 px-3 text-xs text-right font-mono font-bold text-[#2E3093]">{fmt(totalNet)}</td>
          </tr>
        </tfoot>
      </table>
