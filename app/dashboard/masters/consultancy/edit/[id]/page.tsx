...
import DeputationSection from './DeputationSection';
...
export default function EditConsultancyPage() {
  ...
  const consultancyId = Number(id);
  const { canUpdate, loading: permLoading } = useResourcePermissions('consultancy');
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/masters/consultancy/${consultancyId}`);
        const data = await res.json();
        if (!res.ok) throw new Error(data.error);
        const r = data.row;
        setForm({
          Comp_Name: r.Comp_Name || '', Designation: r.Designation || '', City: r.City || '',
          State: r.State || '', Tel: r.Tel || '', Fax: r.Fax || '', Mobile: r.Mobile || '',
          Date_Added: toDateInput(r.Date_Added), Industry: r.Industry || '', Remark: r.Remark || '',
  ...
  const handleSaveDetails = async () => {
    ...
    try {
      const res = await fetch('/api/masters/consultancy', {
        ...
        body: JSON.stringify({ Const_Id: consultancyId, ...form }),
      });
      const data = await res.json();
  ...
  const handleDeleteBranch = async (bid: number) => {
    ...
    await fetch(`/api/masters/consultancy/branches?id=${bid}`, { method: 'DELETE' });
    fetchBranches();
  };
  ...
  const fetchStudents = useCallback(async () => {
    ...
    setStudentsError('');
    try {
      const res = await fetch(`/api/masters/consultancy/students?constId=${consultancyId}`);
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Failed to load student details');
      setStudentRows(data.rows ?? []);
    } catch (err: unknown) {
      setStudentsError(err instanceof Error ? err.message : 'Failed to load student details');
      setStudentRows([]);
    } finally {
  ...
  const handleAddFollowup = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/masters/consultancy/followups', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ constId: consultancyId, id: editingFollowupId, ...followupForm }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Save failed');
      resetFollowupForm();
  ...
  const handleEditFollowup = (f: FollowUp) => {
    if (f.Followup_Id == null) return;
    setDiscussionSourceFollowupId(null);
    setEditingFollowupId(f.Followup_Id);
    setFollowupForm({
      Followup_Date: toDateInput(f.Followup_Date),
      Contact_Person: f.Contact_Person || '',
      Designation: f.Designation || '',
  ...
  const handleDeleteFollowup = async (fid: number) => {
    ...
    await fetch(`/api/masters/consultancy/followups?id=${fid}`, { method: 'DELETE' });
    fetchFollowups();
  };
  ...
  if (permLoading || loadingData) return <PermissionLoading />;
  if (!canUpdate) return <AccessDenied message="You do not have permission to edit consultancies." />;
    ...
    <div className="space-y-3">
      ...
      <div className="bg-gradient-to-r from-[#2E3093] to-[#2A6BB5] rounded-xl px-5 py-4 shadow-md">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-bold text-white">Edit Consultancy Master</h2>
            <p className="text-xs text-white/70">Placement &gt; Consultancy Master &gt; Edit #{consultancyId}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div className="flex border-b border-gray-200 px-5 bg-gray-50/80">
          {(['details', 'student', 'branches', 'followups'] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              ...
              {tab === 'student' && <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.5 20.118a7.5 7.5 0 0115 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.5-1.632z" /></svg>}
        ...
        {success && (
          <div className="mx-5 mt-3 bg-green-50 border border-green-200 rounded-lg px-4 py-2 text-sm text-green-700">{success}</div>
        )}
        ...
        <div className="p-5">
          ...
          {/* =============== STUDENT DETAILS TAB =============== */}
          {activeTab === 'student' && (
            <SectionCard title="Student Details" icon={<svg className="w-3.5 h-3.5 text-[#2E3093]" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.5 20.118a7.5 7.5 0 0115 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.5-1.632z" /></svg>}>
              {studentsError && (
                  ...
                  <table className="dashboard-table w-full text-sm">
                    <thead className="sticky top-0 bg-white">
                      <tr className="bg-gradient-to-r from-[#2E3093]/10 to-[#2A6BB5]/10">
                        ...
                        <th className="text-left px-3 py-2 font-semibold text-[#2E3093] border-b whitespace-nowrap">Year of Passing</th>
                        ...
                        <th className="text-left px-3 py-2 font-semibold text-[#2E3093] border-b whitespace-nowrap">Discipline</th>
                        <th className="text-left px-3 py-2 font-semibold text-[#2E3093] border-b whitespace-nowrap">Placed Date</th>
                        <th className="text-left px-3 py-2 font-semibold text-[#2E3093] border-b whitespace-nowrap">Status</th>
                      </tr>
                    </thead>
                    <tbody>
                      {studentsLoading ? (
                        <tr><td colSpan={9} className="px-4 py-6 text-center text-gray-400 text-xs">Loading…</td></tr>
                      ) : studentRows.length === 0 ? (
                        <tr><td colSpan={9} className="px-4 py-6 text-center text-gray-400 text-xs">No student details found</td></tr>
                      ) : studentRows.map(r => (
                        <tr key={r.id} className="border-b border-gray-100 hover:bg-gray-50">
                          <td className="px-3 py-2 text-gray-700">{r.studentName || '-'}</td>
                          <td className="px-3 py-2 text-gray-700">{r.courseName || '-'}</td>
                          <td className="px-3 py-2 text-gray-700">{r.batchCode || '-'}</td>
                          <td className="px-3 py-2 text-gray-700">{r.yearOfPassing || '-'}</td>
                          ...
                          <td className="px-3 py-2 text-gray-700">{r.placedDate ? toDateInput(r.placedDate) : '-'}</td>
            ...
            <div className="space-y-4">
              ...
              <SectionCard title="Add Branches" icon={<svg className="w-3.5 h-3.5 text-[#2E3093]" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>}>
                ...
                <div className="flex items-center gap-3 mt-3">
                  ...
                  <button onClick={handleSaveBranches} disabled={saving}
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between gap-3 rounded-lg border border-gray-200 bg-white px-4 py-2.5 shadow-sm">
                <div className="flex items-center gap-2">
                  <span className={`inline-flex items-center rounded-md text-[10px] font-bold px-2 py-0.5 ${atsMode ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-500'}`}>ATS</span>
                  <div>
                    <div className="text-sm font-semibold text-gray-900">
                      {atsMode ? 'Deputation Mode' : 'Standard Follow-up Mode'}
                    </div>
                    <div className="text-[11px] text-gray-500">
                      {atsMode
                        ? 'Track agreements, JD status, positions and closing outcomes.'
                        : 'Log standard follow-up touch-points against this consultancy.'}
                    </div>
                  </div>
                </div>
                <button
                  ...
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${atsMode ? 'bg-indigo-600' : 'bg-gray-300'}`}
                >
                  <span className={`inline-block h-5 w-5 transform rounded-full bg-white shadow transition-transform ${atsMode ? 'translate-x-5' : 'translate-x-0.5'}`} />
                </button>
              </div>
              ...
              <SectionCard title="View Consultancy Info" icon={<svg className="w-3.5 h-3.5 text-[#2E3093]" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}>
                {editingFollowupId != null && (
                  <div className="mb-3 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-900 flex items-center justify-between gap-2">
                    ...
                    <button
                      ...
                      className="px-2 py-1 text-[11px] font-semibold rounded border border-blue-300 hover:bg-blue-100"
                    >
                      Cancel Edit
                    </button>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-2 mt-1">
                  ...
                  <div>
                    <label className={labelCls}>Purpose</label>
                    <select className={selectCls} value={followupForm.Purpose} onChange={e => setFollowupForm(f => ({ ...f, Purpose: e.target.value }))}>
                      <option value="">--Select Purpose--</option>
                ...
                <div className="flex justify-end mt-3 gap-2">
                  {(discussionSourceFollowupId != null || editingFollowupId != null) && (
                    <button
                      type="button"
                      onClick={resetFollowupForm}
                        ...
                        >
                          ...
                          <td className="px-3 py-2 text-center">
                            <div className="flex items-center justify-center gap-1">
                              ...
                              {f.Followup_Id != null && (
                                <button onClick={() => handleEditFollowup(f)} className="p-1 text-indigo-700 hover:bg-indigo-50 rounded" title="Edit">
