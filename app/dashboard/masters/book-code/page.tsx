...
export default function BookCodePage() {
  const { canView, canCreate, canUpdate, canDelete, loading: permLoading } = useResourcePermissions('book_code');
  /* ---- Form state ---- */
  ...
  /* ---- List state ---- */
  const [rows, setRows] = useState<BookCode[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/masters/book-code?${params.toString()}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const body = editId 
        ? { id: editId, title: formData.title }
        : { title: formData.title };

      const res = await fetch('/api/masters/book-code', {
        method,
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Are you sure you want to delete this book code?')) return;
    try {
      const res = await fetch(`/api/masters/book-code?id=${id}`, { method: 'DELETE' });
      if (res.ok) {
        fetchData();
      }
    } catch {
  ...
  const handleExport = () => {
    const headers = ['#', 'Title'];
    const csvRows = rows.map(r => [r.id, r.title || '']);
    const csv = [headers.join(','), ...csvRows.map(r => r.join(','))].join('\n');
    const blob = new Blob([csv], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = 'book_code_list.csv';
    link.click();
  ...
  if (!canView) return <AccessDenied message="You do not have permission to view book codes." />;
    ...
    <div className="h-screen overflow-hidden bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/20 p-6 flex flex-col">
      ...
      <div className="mb-6 flex-shrink-0">
        <div className="flex items-center gap-2 text-sm text-gray-500 mb-2">
          ...
          <span className="text-[#2E3093] font-medium">Book Code</span>
        </div>
        <h1 className="text-2xl font-bold text-gray-800">Book Code</h1>
      </div>

      {/* 2-Column Grid */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1 min-h-0 overflow-hidden">
        {/* Column 1: Form */}
        <SectionCard title={editId ? 'Edit Book Code' : 'Add Book Code'}>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                className={inputCls}
                placeholder="Enter Book Code Name"
                required
              />
            </div>
            <div className="flex gap-3 pt-2">
              <button
                type="submit"
                disabled={submitting}
                className="px-4 py-2 bg-gradient-to-r from-[#2E3093] to-[#2A6BB5] text-white text-sm font-semibold rounded-lg shadow hover:shadow-md transition-all disabled:opacity-50"
              >
        ...
        {/* Column 2: List */}
        <SectionCard title="List Of Book Code" contentClassName="flex flex-col min-h-0">
          {/* Toolbar */}
          <div className="flex flex-wrap items-center gap-2 mb-4 flex-shrink-0">
            ...
            <div className="relative">
              <input
                ...
                className="w-40 pl-8 pr-3 py-1.5 border border-gray-300 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-[#2E3093]/20 focus:border-[#2E3093]"
              />
              ...
              <table className="dashboard-table w-full text-sm">
                <thead className="sticky top-0 z-10 bg-white">
                  <tr className="bg-gradient-to-r from-[#2E3093]/10 to-[#2A6BB5]/10">
                    <th className="text-left px-4 py-2.5 font-semibold text-[#2E3093] border-b whitespace-nowrap">#</th>
                    <th className="text-left px-4 py-2.5 font-semibold text-[#2E3093] border-b whitespace-nowrap">Title</th>
                    <th className="text-center px-4 py-2.5 font-semibold text-[#2E3093] border-b whitespace-nowrap">Action</th>
                  </tr>
                </thead>
                <tbody>
                  {loading ? (
                    ...
                    rows.map((row) => (
                      <tr key={row.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                        <td className="px-4 py-2.5 text-gray-700">{row.id}</td>
                        <td className="px-4 py-2.5 text-gray-900 font-medium">{row.title || '-'}</td>
