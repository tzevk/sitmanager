...
export default function StatusPage() {
  const { canView, canCreate, canUpdate, canDelete, loading: permLoading } = useResourcePermissions('status');
  /* ---- Form state ---- */
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/masters/status?${params.toString()}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const body = editId 
        ? { id: editId, status: formData.status, description: formData.description }
        : { status: formData.status, description: formData.description };

      const res = await fetch('/api/masters/status', {
        method,
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/masters/status?id=${id}`, { method: 'DELETE' });
      if (res.ok) {
        fetchData();
      }
    } catch {
  ...
  const handleExport = () => {
    ...
    const csvRows = rows.map(r => [r.Id, r.Status || '', r.Description || '']);
    const csv = [headers.join(','), ...csvRows.map(r => r.join(','))].join('\n');
    const blob = new Blob([csv], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = 'status_list.csv';
    link.click();
  ...
  if (!canView) return <AccessDenied message="You do not have permission to view statuses." />;
    ...
    <div className="h-screen overflow-hidden bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/20 p-6 flex flex-col">
      ...
      <div className="mb-6 flex-shrink-0">
        <div className="flex items-center gap-2 text-sm text-gray-500 mb-2">
          ...
          <span className="text-[#2E3093] font-medium">Status</span>
        </div>
        <h1 className="text-2xl font-bold text-gray-800">Status Master</h1>
      </div>

      {/* 2-Column Grid */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1 min-h-0 overflow-hidden">
        {/* Column 1: Form */}
        <SectionCard title={editId ? 'Edit Status' : 'Add Status'}>
        ...
        {/* Column 2: List */}
        <SectionCard title="List of Status" contentClassName="flex flex-col min-h-0">
          {/* Toolbar */}
              ...
              <table className="dashboard-table w-full text-sm">
                <thead className="sticky top-0 z-10 bg-white">
                  <tr className="bg-gradient-to-r from-[#2E3093]/10 to-[#2A6BB5]/10">
                    <th className="text-left px-4 py-2.5 font-semibold text-[#2E3093] border-b whitespace-nowrap">Id</th>
                    <th className="text-left px-4 py-2.5 font-semibold text-[#2E3093] border-b whitespace-nowrap">Status</th>
                    <th className="text-left px-4 py-2.5 font-semibold text-[#2E3093] border-b whitespace-nowrap">Description</th>
                  ...
                  {loading ? (
                    ...
                    rows.map((row) => (
                      <tr key={row.Id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-2.5 text-gray-900 font-medium">{row.Status || '-'}</td>
