...
export default function CollegePage() {
  ...
  const { canView, canCreate, canUpdate, canDelete, loading: permLoading } = useResourcePermissions('college');
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/masters/college?${params.toString()}`);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/masters/college?id=${id}`, { method: 'DELETE' });
      if (res.ok) {
        fetchData();
      }
    } catch {
  ...
  const handleExport = () => {
    const headers = ['Id', 'College_Name', 'University', 'Contact_Person', 'Designation', 'Address', 'City'];
    const csvRows = rows.map(r => [r.id, r.college_name || '', r.university || '', r.contact_person || '', r.designation || '', r.address || '', r.city || '']);
    const csv = [headers.join(','), ...csvRows.map(r => r.map(v => `"${String(v).replace(/"/g, '""')}"`).join(','))].join('\n');
    const blob = new Blob([csv], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = 'college_list.csv';
    link.click();
  ...
  if (!canView) return <AccessDenied message="You do not have permission to view colleges." />;
    ...
    <div className="h-screen overflow-hidden bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/20 p-6 flex flex-col">
      ...
      <div className="mb-6 flex-shrink-0 flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 text-sm text-gray-500 mb-2">
            ...
            <span className="text-[#2E3093] font-medium">College</span>
          </div>
          <h1 className="text-2xl font-bold text-gray-800">College</h1>
        </div>
        {canCreate && (
        <button
          onClick={() => router.push('/dashboard/masters/college/add')}
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col flex-1 min-h-0">
        <div className="bg-gradient-to-r from-[#2E3093] to-[#2A6BB5] px-5 py-3 flex-shrink-0">
          <h3 className="text-sm font-bold text-white tracking-wide">List Of College</h3>
        </div>
              ...
              <table className="dashboard-table w-full text-sm">
                <thead className="sticky top-0 z-10 bg-white">
                  <tr className="bg-gradient-to-r from-[#2E3093]/10 to-[#2A6BB5]/10">
                    ...
                    <th className="text-left px-3 py-2 font-semibold text-[#2E3093] border-b whitespace-nowrap">College_Name</th>
                    <th className="text-left px-3 py-2 font-semibold text-[#2E3093] border-b whitespace-nowrap">University</th>
                    <th className="text-left px-3 py-2 font-semibold text-[#2E3093] border-b whitespace-nowrap">Contact_Person</th>
                  ...
                  {loading ? (
                    ...
                    rows.map((row) => (
                      <tr key={row.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                        <td className="px-3 py-2.5 text-gray-700">{row.id}</td>
                        <td className="px-3 py-2.5 text-gray-900 font-medium truncate max-w-[150px]" title={row.college_name || ''}>{row.college_name || '-'}</td>
                        <td className="px-3 py-2.5 text-gray-700 truncate max-w-[120px]" title={row.university || ''}>{row.university || '-'}</td>
                        <td className="px-3 py-2.5 text-gray-700 truncate max-w-[100px]" title={row.contact_person || ''}>{row.contact_person || '-'}</td>
                        <td className="px-3 py-2.5 text-gray-700 truncate max-w-[100px]" title={row.designation || ''}>{row.designation || '-'}</td>
                        <td className="px-3 py-2.5 text-gray-700 truncate max-w-[120px]" title={row.address || ''}>{row.address || '-'}</td>
