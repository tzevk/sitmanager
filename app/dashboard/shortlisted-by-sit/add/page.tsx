...
export default function AddShortlistedBySitPage() {
  const router = useRouter();
  const { canCreate, loading: permLoading } = useResourcePermissions('shortlisted_sit');
    ...
    try {
      const res = await fetch('/api/placement/shortlisted-by-sit', {
        ...
        body: JSON.stringify({
          ...
          batches: batchRows.filter(b => b.BatchId).map(b => ({ BatchId: b.BatchId })),
        }),
      });
      if (res.ok) {
  ...
  if (!canCreate) return <AccessDenied message="You do not have permission to create Shortlisted By SIT records." />;
      ...
      <div className="mb-4 flex-shrink-0 flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 text-sm text-gray-500 mb-1">
            ...
            <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" /></svg>
            <span className="text-[#2E3093] font-medium">Add New</span>
          </div>
          <h1 className="text-2xl font-bold text-gray-800">Add Shortlisted By SIT</h1>
        </div>
