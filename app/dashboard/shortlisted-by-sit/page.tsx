...
export default function ShortlistedBySitPage() {
  const router = useRouter();
  const { canView, canCreate, canUpdate, canDelete, loading: permLoading } = useResourcePermissions('shortlisted_sit');
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/placement/shortlisted-by-sit?${params.toString()}`);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/placement/shortlisted-by-sit?id=${id}`, { method: 'DELETE' });
      if (res.ok) fetchData();
    } catch { /* ignore */ }
  };

  const handleExport = () => {
    const headers = ['Company Name', 'Posted Date', 'Course', 'Profile', 'Location'];
    const csvRows = rows.map(r => [
      r.Comp_Name || '', r.PostedDate || '', r.Course_Name || '', r.Profile || '', r.Location || '',
    ]);
    const csv = [headers.join(','), ...csvRows.map(r => r.map(v => `"${String(v).replace(/"/g, '""')}"`).join(','))].join('\n');
    const blob = new Blob([csv], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = 'shortlisted_by_sit.csv'; a.click();
  ...
  if (!canView) return <AccessDenied message="You do not have permission to view Shortlisted By SIT." />;
    ...
    <div className="h-screen overflow-hidden bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/20 p-6 flex flex-col">
      ...
      <div className="mb-6 flex-shrink-0 flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 text-sm text-gray-500 mb-2">
            ...
            <span>Placement</span>
            <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" /></svg>
            <span className="text-[#2E3093] font-medium">Shortlisted By SIT</span>
          </div>
          <h1 className="text-2xl font-bold text-gray-800">Shortlisted By SIT</h1>
        </div>
        {canCreate && (
          <button
            onClick={() => router.push('/dashboard/shortlisted-by-sit/add')}
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col flex-1 min-h-0">
        <div className="bg-gradient-to-r from-[#2E3093] to-[#2A6BB5] px-5 py-3 flex-shrink-0">
          <h3 className="text-sm font-bold text-white tracking-wide">List Of Shortlisted By SIT</h3>
        </div>
              ...
              <table className="dashboard-table w-full text-sm">
                <thead className="sticky top-0 z-10 bg-white">
                  <tr className="bg-gradient-to-r from-[#2E3093]/10 to-[#2A6BB5]/10">
                    ...
                    <th className="text-left px-3 py-2 font-semibold text-[#2E3093] border-b whitespace-nowrap">Posted Date</th>
                    <th className="text-left px-3 py-2 font-semibold text-[#2E3093] border-b whitespace-nowrap">Course</th>
                    <th className="text-left px-3 py-2 font-semibold text-[#2E3093] border-b whitespace-nowrap">Profile</th>
                    <th className="text-left px-3 py-2 font-semibold text-[#2E3093] border-b whitespace-nowrap">Location</th>
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr key={row.CompReqId} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                      <td className="px-3 py-2.5 text-gray-900 font-medium truncate max-w-[200px]" title={row.Comp_Name || ''}>{row.Comp_Name || '-'}</td>
                      <td className="px-3 py-2.5 text-gray-700">{formatDate(row.PostedDate)}</td>
                      <td className="px-3 py-2.5 text-gray-700 truncate max-w-[180px]" title={row.Course_Name || ''}>{row.Course_Name || '-'}</td>
                      <td className="px-3 py-2.5 text-gray-700 truncate max-w-[180px]" title={row.Profile || ''}>{row.Profile || '-'}</td>
                      <td className="px-3 py-2.5 text-gray-700 truncate max-w-[150px]" title={row.Location || ''}>{row.Location || '-'}</td>
                      <td className="px-3 py-2.5 text-center">
                        <div className="flex items-center justify-center gap-1">
                          {canUpdate && (
                            <button onClick={() => router.push(`/dashboard/shortlisted-by-sit/edit/${row.CompReqId}`)}
                          ...
                          {canDelete && (
                            <button onClick={() => handleDelete(row.CompReqId)}
