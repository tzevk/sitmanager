...
export default function MetaLeadsPage() {
  ...
  useEffect(() => {
    ...
    const exists = rows.some((r) => r.MetaLead_Id === editingLeadId);
    if (!exists) setEditingLeadId(null);
  }, [editingLeadId, rows]);
  ...
  const startRowEdit = useCallback((row: InquiryRow) => {
    if (!row.MetaLead_Id || !canUpdate) return;
    setConvertError('');
    setRowDrafts((prev) => ({
      ...prev,
    ...
    setEditingLeadId(row.MetaLead_Id);
  }, [canUpdate]);

  const cancelRowEdit = useCallback((row: InquiryRow) => {
    if (!row.MetaLead_Id) return;
    setRowDrafts((prev) => ({
      ...prev,
      [row.MetaLead_Id]: {
  ...
  // How many pages to fetch at once. Each individual request is still the
  // same <=100-row query the table itself uses (see the API route's
  // maxLimit comment for why that per-request cap exists) — this only
  // controls how many of those already-safe requests run concurrently, to
  // cut wall-clock time on large exports (e.g. ~11k leads = ~111 pages,
  // which at one-at-a-time took minutes).
  ...
  const exportLeadsCsv = useCallback(async (tab: 'all' | 'fresh' | 'engaged') => {
    ...
    try {
      ...
      const fetchPage = async (pageNum: number) => {
        const p = new URLSearchParams({ ...baseParams, page: String(pageNum) });
        const res = await fetch(`/api/meta-ads/leads?${p.toString()}`);
        const data = await res.json().catch(() => ({}));
        if (!res.ok) throw new Error(data?.error || 'Failed to export Meta leads');
        return data as { rows?: InquiryRow[]; pagination?: { totalPages?: number } };
      };

      const first = await fetchPage(1);
      const totalPages = first.pagination?.totalPages ?? 1;
      const pagesByIndex: InquiryRow[][] = new Array(totalPages);
      pagesByIndex[0] = first.rows ?? [];
      setExportProgress({ done: 1, total: totalPages });

      let nextPage = 2;
      let doneCount = 1;
      const workers = Array.from({ length: Math.min(EXPORT_CONCURRENCY, Math.max(0, totalPages - 1)) }, async () => {
        while (nextPage <= totalPages) {
          const pageNum = nextPage++;
          const data = await fetchPage(pageNum);
          pagesByIndex[pageNum - 1] = data.rows ?? [];
          doneCount += 1;
          setExportProgress({ done: doneCount, total: totalPages });
        }
      });
      await Promise.all(workers);

      const all = pagesByIndex.flat();
                  ...
                  <table className="w-full text-xs border-collapse">
                    ...
                    <tbody>
                      {loading ? (
                        ...
                        const isEditing = editingLeadId === row.MetaLead_Id;
                        const inputDisabled = !canUpdate || isSaving || !isEditing;
                                ...
                                {isEditing ? (
                                  <>
                                    ...
                                    <button
                                      ...
                                      disabled={isSaving}
                                      className="inline-flex items-center rounded-md border border-slate-200 bg-slate-50 px-2 py-1 text-[10px] font-semibold text-slate-600 transition-colors hover:bg-slate-100 disabled:cursor-not-allowed disabled:text-slate-300"
                                    >Cancel</button>
                                  </>
                                ) : (
                                  <button
                                    ...
                                    disabled={!canUpdate || !row.MetaLead_Id || Boolean(editingLeadId && editingLeadId !== row.MetaLead_Id)}
                                    className={`inline-flex items-center rounded-md px-2 py-1 text-[10px] font-semibold transition-colors ${canUpdate && row.MetaLead_Id ? 'border border-slate-200 bg-white text-slate-700 hover:bg-slate-50' : 'border border-slate-200 bg-slate-50 text-slate-300 cursor-not-allowed'}`}
                                  >Edit</button>
                                )}
