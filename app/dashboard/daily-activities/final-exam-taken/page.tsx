...
interface FinalExam {
  ...
  FE_MaxMarks: string | null;
  FE_Duration: string | null;
  Exam_Dt: string | null;
}
...
/*  Page                                                               */
/* ------------------------------------------------------------------ */
export default function FinalExamTakenPage() {
  return (
    <PermissionGate resource="final_exam" deniedMessage="You do not have permission to view final exams.">
      {(perms) => <FinalExamTakenContent {...perms} />}
    </PermissionGate>
  );
}

function FinalExamTakenContent({ canCreate, canUpdate, canDelete }: { canView: boolean; canCreate: boolean; canUpdate: boolean; canDelete: boolean; canExport: boolean }) {
  const router = useRouter();

  /* ---- Data ---- */
  const [rows, setRows] = useState<FinalExam[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/daily-activities/final-exam-taken?${params}`);
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Are you sure you want to delete this final exam record?')) return;
    setDeleting(id);
    try {
      const res = await fetch(`/api/daily-activities/final-exam-taken?id=${id}`, { method: 'DELETE' });
      if (!res.ok) throw new Error('Failed to delete');
      setFetchTrigger(t => t + 1);
    } catch { alert('Failed to delete final exam record'); }
    setDeleting(null);
  };
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `final-exam-taken-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <h1 className="text-xl font-bold text-gray-800 tracking-tight">Final Exam Taken</h1>
          <p className="text-xs text-gray-400">Daily Activities / Final Exam Taken</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between gap-3 flex-wrap flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            {canCreate && (
            <button
              onClick={() => router.push('/dashboard/daily-activities/final-exam-taken/add')}
                ...
                <tr>
                  <td colSpan={5} className="py-16 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-sm text-gray-400">No final exam records found</p>
                    </div>
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={r.Take_Id} className="border-b border-gray-50 hover:bg-blue-50/30 transition-colors">
                    ...
                    <td className="py-2.5 px-4 text-center">
                      <div className="flex items-center justify-center gap-1">
                        {canUpdate && (
                        <button
                          onClick={() => router.push(`/dashboard/daily-activities/final-exam-taken/add?id=${r.Take_Id}`)}
