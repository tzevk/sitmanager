...
/*  Page                                                               */
/* ------------------------------------------------------------------ */
export default function AssignmentsTakenPage() {
  return (
    <PermissionGate resource="assignment" deniedMessage="You do not have permission to view assignments.">
      {(perms) => <AssignmentsTakenContent {...perms} />}
    </PermissionGate>
  );
}

function AssignmentsTakenContent({ canCreate, canUpdate, canDelete }: { canView: boolean; canCreate: boolean; canUpdate: boolean; canDelete: boolean; canExport: boolean }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/daily-activities/assignments-taken?${params}`);
  ...
  const handleDelete = async (id: number) => {
    ...
    setDeleting(id);
    try {
      const res = await fetch(`/api/daily-activities/assignments-taken?id=${id}`, { method: 'DELETE' });
      if (!res.ok) throw new Error('Failed to delete');
      setFetchTrigger(t => t + 1);
    } catch { alert('Failed to delete assignment record'); }
    setDeleting(null);
  };
  ...
  const handleExport = () => {
    ...
    const csvRows = [
      ...
      ...rows.map(r => [
        ...
        `"${(r.AssignmentName || '').replace(/"/g, '""')}"`,
        `"${formatDate(r.Assign_Dt)}"`,
        `"${formatDate(r.Return_Dt)}"`,
        r.Marks ?? '',
    ...
    a.href = url;
    a.download = `assignments-taken-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <h1 className="text-xl font-bold text-gray-800 tracking-tight">Assignments Taken</h1>
          <p className="text-xs text-gray-400">Daily Activities / Assignments Taken</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between gap-3 flex-wrap flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            {canCreate && (
            <button
              onClick={() => router.push('/dashboard/daily-activities/assignments-taken/add')}
                ...
                <tr>
                  <td colSpan={7} className="py-16 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-sm text-gray-400">No assignment records found</p>
                    </div>
                  </td>
                </tr>
              ) : (
                rows.map((r) => (
                  <tr key={r.Given_Id} className="border-b border-gray-50 hover:bg-blue-50/30 transition-colors">
                    ...
                    <td className="py-2.5 px-4">
                      <div className="flex flex-col">
                        <span className="font-semibold text-gray-800 text-sm">
                          {r.AssignmentName || `#${r.Assign_No || '—'}`}
                        </span>
                        {r.subjects && r.subjects !== 'Assignment' && (
                          <span className="text-[11px] text-gray-400 mt-0.5">{r.subjects}</span>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="py-2.5 px-4 text-center">
                      <div className="flex items-center justify-center gap-1">
                        {canUpdate && (
                        <button
                          onClick={() => router.push(`/dashboard/daily-activities/assignments-taken/add?id=${r.Given_Id}`)}
                        ...
                        {canDelete && (
                        <button
                          onClick={() => handleDelete(r.Given_Id)}
                          disabled={deleting === r.Given_Id}
