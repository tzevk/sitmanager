...
/*  Page                                                               */
/* ------------------------------------------------------------------ */
export default function VivaMocTakenPage() {
  return (
    <PermissionGate resource="viva_moc" deniedMessage="You do not have permission to view Viva/MOC records.">
      {(perms) => <VivaMocTakenContent {...perms} />}
    </PermissionGate>
  );
}

function VivaMocTakenContent({ canCreate, canUpdate, canDelete }: { canView: boolean; canCreate: boolean; canUpdate: boolean; canDelete: boolean; canExport: boolean }) {
  const router = useRouter();

  /* ---- Data ---- */
  const [rows, setRows] = useState<VivaMoc[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/daily-activities/viva-moc-taken?${params}`);
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Are you sure you want to delete this Viva/MOC record?')) return;
    setDeleting(id);
    try {
      const res = await fetch(`/api/daily-activities/viva-moc-taken?id=${id}`, { method: 'DELETE' });
      if (!res.ok) throw new Error('Failed to delete');
      setFetchTrigger(t => t + 1);
    } catch { alert('Failed to delete Viva/MOC record'); }
    setDeleting(null);
  };
  ...
  const handleExport = () => {
    ...
    const csvRows = [
      ...
      ...rows.map(r => [
        r.id,
        `"${(r.Batch_code_display || r.batchcode || '').replace(/"/g, '""')}"`,
        `"${(r.Course_Name || '').replace(/"/g, '""')}"`,
        `"${(r.vivamocname || '').replace(/"/g, '""')}"`,
        `"${formatDate(r.date)}"`,
        r.marks ?? '',
      ].join(',')),
    ];
    ...
    a.href = url;
    a.download = `viva-moc-taken-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <h1 className="text-xl font-bold text-gray-800 tracking-tight">Viva / MOC Taken</h1>
          <p className="text-xs text-gray-400">Daily Activities / Viva / MOC Taken</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between gap-3 flex-wrap flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            {canCreate && (
            <button
              onClick={() => router.push('/dashboard/daily-activities/viva-moc-taken/add')}
                ...
                <tr>
                  <td colSpan={6} className="py-16 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-sm text-gray-400">No Viva/MOC records found</p>
                    </div>
                  </td>
                </tr>
              ) : (
