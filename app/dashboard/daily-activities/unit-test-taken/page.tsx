...
interface UnitTest {
  ...
  Test_No: number | null;
  Marks: number | null;
  UT_Marks: string | null;
  UT_Duration: string | null;
  Test_Dt: string | null;
  studentCount: number;
}
...
/*  Page                                                               */
/* ------------------------------------------------------------------ */
export default function UnitTestTakenPage() {
  return (
    <PermissionGate resource="unit_test" deniedMessage="You do not have permission to view unit tests.">
      {(perms) => <UnitTestTakenContent {...perms} />}
    </PermissionGate>
  );
}

function UnitTestTakenContent({ canCreate, canUpdate, canDelete }: { canView: boolean; canCreate: boolean; canUpdate: boolean; canDelete: boolean; canExport: boolean }) {
  const router = useRouter();

  /* ---- Data ---- */
  const [rows, setRows] = useState<UnitTest[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/daily-activities/unit-test-taken?${params}`);
  ...
  const handleViewStudents = async (row: UnitTest) => {
    const { Take_Id, Batch_Id } = row;
    if (expandedTakeId === Take_Id) { setExpandedTakeId(null); return; }
    setExpandedTakeId(Take_Id);
    if (studentsMap[Take_Id]) return; // already fetched
    if (!Batch_Id) { setStudentsMap(m => ({ ...m, [Take_Id]: [] })); return; }
    setStudentsLoading(Take_Id);
    try {
      const res = await fetch(
        `/api/daily-activities/unit-test-taken?options=students&batchId=${Batch_Id}&takeId=${Take_Id}`
      );
      if (!res.ok) throw new Error();
      const data = await res.json();
      setStudentsMap(m => ({ ...m, [Take_Id]: data.students ?? [] }));
    } catch {
      setStudentsMap(m => ({ ...m, [Take_Id]: [] }));
    }
    setStudentsLoading(null);
  };

  const handleDelete = async (id: number) => {
    if (!confirm('Are you sure you want to delete this unit test record?')) return;
    setDeleting(id);
    try {
      const res = await fetch(`/api/daily-activities/unit-test-taken?id=${id}`, { method: 'DELETE' });
      if (!res.ok) throw new Error('Failed to delete');
      setFetchTrigger(t => t + 1);
    } catch { alert('Failed to delete unit test record'); }
    setDeleting(null);
  };
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `unit-test-taken-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <h1 className="text-xl font-bold text-gray-800 tracking-tight">Unit Test Taken</h1>
          <p className="text-xs text-gray-400">Daily Activities / Unit Test Taken</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between gap-3 flex-wrap flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            {canCreate && (
            <button
              onClick={() => router.push('/dashboard/daily-activities/unit-test-taken/add')}
                ...
                <tr>
                  <td colSpan={8} className="py-16 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-sm text-gray-400">No unit test records found</p>
                    </div>
                  </td>
                </tr>
              ) : (
                rows.map((r) => (
                  <React.Fragment key={r.Take_Id}>
                    <tr className={`border-b border-gray-50 hover:bg-blue-50/30 transition-colors ${expandedTakeId === r.Take_Id ? 'bg-blue-50/40' : ''}`}>
                      ...
                      <td className="py-2.5 px-4">
                        <div className="flex flex-col">
                          <span className="font-semibold text-gray-800 text-sm">
                            {r.TestName || `Test #${r.Test_No || '—'}`}
                          </span>
                          {r.UT_Duration && (
                            <span className="text-[11px] text-gray-400 mt-0.5">Duration: {r.UT_Duration}</span>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="py-2.5 px-4 text-center">
                        ...
                        >
                          ...
                          {r.studentCount > 0 ? r.studentCount : 'View'}
                        </button>
                      </td>
                      <td className="py-2.5 px-4 text-center">
                        <div className="flex items-center justify-center gap-1">
                          {canUpdate && (
                          <button
                            onClick={() => router.push(`/dashboard/daily-activities/unit-test-taken/add?id=${r.Take_Id}`)}
                          ...
                          <div className="px-6 py-4">
                            <div className="flex items-center gap-2 mb-3">
                              ...
                              <span className="text-xs font-semibold text-[#2E3093]">
                                Students — {r.Batch_code} &nbsp;·&nbsp; {r.TestName || `Test #${r.Test_No}`} &nbsp;·&nbsp; Max Marks: {r.Marks ?? '—'}
                              </span>
                            </div>
                            ...
                            ) : (studentsMap[r.Take_Id] ?? []).length === 0 ? (
                              ...
                              <div className="overflow-x-auto rounded-lg border border-gray-200">
                                <table className="w-full text-xs">
                                  ...
                                  <tbody className="divide-y divide-gray-100 bg-white">
                                    {(studentsMap[r.Take_Id] ?? []).map((s) => {
                                      const pct = s.marks_obtained != null && r.Marks
                                        ? Math.round((s.marks_obtained / r.Marks) * 100)
                                        : null;
                                      return (
                                        <tr key={s.Student_Id} className="hover:bg-blue-50/20 transition-colors">
                                          ...
                                          <td className="py-1.5 px-3 text-center text-gray-500">{r.Marks ?? '—'}</td>
                                          <td className="py-1.5 px-3 text-center">
                                            {pct != null ? (
                                              <span className={`text-xs font-semibold ${pct >= 75 ? 'text-green-600' : pct >= 50 ? 'text-amber-600' : 'text-red-500'}`}>
                                                {pct}%
                                              </span>
                                            ) : (
                                              <span className="text-gray-300">—</span>
