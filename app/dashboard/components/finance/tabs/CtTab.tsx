'use client';

import { useCallback, useMemo, useState } from 'react';
import { useFinanceResource } from '../shared/useFinanceResource';
import { Modal, TableHeader, TableSkeleton, EmptyRow, RowActions, TotalRow, thCls, tdCls, tdNum, inpCls, lblCls, trCls, PctBar } from '../shared/primitives';
import { fmt, pct, monthLabel, parseMonth } from '../shared/format';
import type { CtRow } from '../shared/types';
import { targetAttainmentTrend } from '../shared/predictions';

export default function CtTab() {
  const ct = useFinanceResource<CtRow>('/api/finance/ct-performance');

  const [modal, setModal] = useState<{ open: boolean; editing: CtRow | null }>({ open: false, editing: null });
  const [form, setForm]   = useState({ month: '', training_name: '', count: '', cost: '', target: '' });
  const [saving, setSaving] = useState(false);

  const openAdd = useCallback(() => {
    setForm({ month: '', training_name: '', count: '', cost: '', target: '' });
    setModal({ open: true, editing: null });
  }, []);
  const openEdit = useCallback((r: CtRow) => {
    setForm({
      month: parseMonth(r.month), // tolerate legacy free-text months
      training_name: r.training_name ?? '',
      count: String(r.count),
      cost: String(r.cost),
      target: String(r.target),
    });
    setModal({ open: true, editing: r });
  }, []);
  const save = useCallback(async () => {
    setSaving(true);
    try {
      await ct.save({
        month: form.month, // ISO YYYY-MM thanks to <input type="month">
        training_name: form.training_name.trim(),
        count: Number(form.count),
        cost: Number(form.cost),
        target: Number(form.target),
      } as Partial<CtRow>, modal.editing);
      setModal({ open: false, editing: null });
    } catch { /* toast */ }
    setSaving(false);
  }, [ct, form, modal.editing]);

  // Sort by ISO month descending (legacy values fall to the end)
  const sortedRows = useMemo(() => {
    return [...ct.rows].sort((a, b) => {
      const am = parseMonth(a.month), bm = parseMonth(b.month);
      return bm.localeCompare(am);
    });
  }, [ct.rows]);

  const totals = useMemo(() => {
    const cost   = sortedRows.reduce((s, r) => s + Number(r.cost   || 0), 0);
    const target = sortedRows.reduce((s, r) => s + Number(r.target || 0), 0);
    const count  = sortedRows.reduce((s, r) => s + Number(r.count  || 0), 0);
    return { cost, target, count };
  }, [sortedRows]);

  // Sort ascending for trend (oldest first)
  const attainmentTrend = useMemo(() =>
    targetAttainmentTrend(
      [...sortedRows]
        .sort((a, b) => parseMonth(a.month).localeCompare(parseMonth(b.month)))
        .map(r => ({ actual: Number(r.cost), target: Number(r.target) }))
    ),
    [sortedRows]
  );

  const trendColor  = attainmentTrend?.direction === 'improving' ? 'text-emerald-700 bg-emerald-50 border-emerald-200'
                     : attainmentTrend?.direction === 'declining'  ? 'text-red-600 bg-red-50 border-red-200'
                     : 'text-amber-700 bg-amber-50 border-amber-200';
  const trendIcon   = attainmentTrend?.direction === 'improving' ? '↑' : attainmentTrend?.direction === 'declining' ? '↓' : '→';

  return (
    <div className="space-y-4">
      <TableHeader title="Corporate Training — Monthly Performance" onAdd={openAdd} />

      {attainmentTrend && (
        <div className={`flex flex-wrap items-center gap-4 rounded-xl border px-4 py-3 text-xs ${trendColor}`}>
          <span className="flex items-center gap-1.5 font-semibold">
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
            </svg>
            AI Trend Insight
          </span>
          <span>Latest attainment: <strong>{attainmentTrend.latestPct.toFixed(1)}%</strong></span>
          <span>Avg: <strong>{attainmentTrend.avgPct.toFixed(1)}%</strong></span>
          <span>Trend: <strong>{trendIcon} {attainmentTrend.direction}</strong> ({attainmentTrend.slopePerPeriod > 0 ? '+' : ''}{attainmentTrend.slopePerPeriod.toFixed(1)}pp/mo)</span>
          <span>Next month forecast: <strong>{attainmentTrend.nextPeriodForecast.toFixed(1)}%</strong></span>
        </div>
      )}
      <div className="overflow-x-auto rounded-xl border border-gray-200">
        <table className="w-full border-collapse">
          <thead><tr className="bg-[#2E3093]">
            <th className={thCls}>Month</th>
            <th className={thCls}>Training Name</th>
            <th className={`${thCls} text-center`}>Conducted (No.)</th>
            <th className={`${thCls} text-center`}>Conducted Cost (₹)</th>
            <th className={`${thCls} text-center`}>Conducted Target (₹)</th>
            <th className={`${thCls} text-center`}>%age</th>
            <th className={`${thCls} text-center`}>Actions</th>
          </tr></thead>
          <tbody className="divide-y divide-gray-100">
            {ct.loading ? <TableSkeleton cols={7} /> :
             sortedRows.length === 0 ? <EmptyRow cols={7} /> :
             sortedRows.map((r, i) => (
              <tr key={r.id} className={trCls(i)}>
                <td className={tdCls}>{monthLabel(parseMonth(r.month))}</td>
                <td className={tdCls}>{r.training_name}</td>
                <td className={tdNum}>{r.count}</td>
                <td className={tdNum}>{fmt(r.cost)}</td>
                <td className={tdNum}>{fmt(r.target)}</td>
                <td className={tdNum}><PctBar value={r.cost} denominator={r.target} /></td>
                <RowActions onEdit={() => openEdit(r)} onDelete={() => ct.remove(r.id)} />
              </tr>
            ))}
            {sortedRows.length > 1 && (
              <TotalRow>
                <td colSpan={2} className="px-3 py-2 text-xs text-[#2E3093]">Total</td>
                <td className="px-3 py-2 text-xs text-center text-[#2E3093]">{totals.count}</td>
                <td className="px-3 py-2 text-xs text-center text-[#2E3093]">{fmt(totals.cost)}</td>
                <td className="px-3 py-2 text-xs text-center text-[#2E3093]">{fmt(totals.target)}</td>
                <td className="px-3 py-2 text-xs text-center text-[#2E3093]">{pct(totals.cost, totals.target)}</td>
                <td />
              </TotalRow>
            )}
          </tbody>
        </table>
      </div>

      <Modal
        open={modal.open}
        title={modal.editing ? 'Edit CT Record' : 'Add CT Record'}
        saving={saving}
        onClose={() => setModal({ open: false, editing: null })}
        onSave={save}
      >
        <div className="grid grid-cols-2 gap-3">
          <div><label className={lblCls}>Month</label><input type="month" className={inpCls} value={form.month} onChange={e => setForm(f => ({ ...f, month: e.target.value }))} /></div>
          <div><label className={lblCls}>Trainings Conducted (No.)</label><input type="number" min="0" className={inpCls} value={form.count} onChange={e => setForm(f => ({ ...f, count: e.target.value }))} /></div>
        </div>
        <div><label className={lblCls}>Training Name</label><input className={inpCls} value={form.training_name} onChange={e => setForm(f => ({ ...f, training_name: e.target.value }))} /></div>
        <div className="grid grid-cols-2 gap-3">
          <div><label className={lblCls}>Conducted Cost (₹)</label><input type="number" min="0" className={inpCls} value={form.cost} onChange={e => setForm(f => ({ ...f, cost: e.target.value }))} /></div>
          <div><label className={lblCls}>Conducted Target (₹)</label><input type="number" min="0" className={inpCls} value={form.target} onChange={e => setForm(f => ({ ...f, target: e.target.value }))} /></div>
        </div>
      </Modal>
    </div>
  );
}
