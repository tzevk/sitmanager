...
export interface LegacyStudentSyncOptions {
  batchSize?: number;
  dryRun?: boolean;
}

export interface LegacyStudentSyncTableSummary {
  sourceTable: string;
  targetTable: string;
  fetched: number;
  upserted: number;
...
export interface LegacyStudentSyncSummary {
  configured: boolean;
  dryRun: boolean;
  tables: LegacyStudentSyncTableSummary[];
  totalFetched: number;
...
const TABLES = [
  {
    sourcePreferred: 'Student_Master',
    sourceLower: 'student_master',
    targetPreferred: 'student_master',
    targetLower: 'student_master',
  },
  {
    sourcePreferred: 'Admission_Master',
    sourceLower: 'admission_master',
    targetPreferred: 'admission_master',
    targetLower: 'admission_master',
  },
];
...
export async function syncLegacyStudentData(options: LegacyStudentSyncOptions = {}): Promise<LegacyStudentSyncSummary> {
  const summary: LegacyStudentSyncSummary = {
    configured: false,
  ...
  try {
    ...
    console.error('[legacy-student-sync] error:', msg);
  }
