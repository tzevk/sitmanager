...
const sql = `
  ...
  LEFT JOIN awt_inquirydiscussion d_inq ON d_inq.deleted = 0 AND d_inq.Inquiry_id = si.Inquiry_Id
...
async function main() {
  ...
  console.log(JSON.stringify(rows.map(r => ({table: r.table, type: r.type, key: r.key, possible: r.possible_keys, rows: r.rows, Extra: r.Extra})), null, 2));
  await conn.end();
}
